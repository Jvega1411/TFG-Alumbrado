...
| `scripts/node-config/hmi_manual_diagnostic.py` | RPi/Lenovo diagnostics | Passive HMI manual troubleshooting: RPi raw/diff FINS reads for HMI screens 1/2, section bits, WR outputs and reset/fotocell/mode context; Lenovo DB inspection for recent cycles/section flags. | 4 | 4 | Read-only. Use during controlled HMI tests before changing PLC semantics. |
